...
router.get('/:user_id',
    ...
    expressAsyncHandler((req, res) => {
        ...
        db.execute(sqlQuery, [parseInt(user_id)], (err, dbResults) => {
            if (err) {
    ...
    expressAsyncHandler((req, res) => {
        ...
        db.execute(`
            ...
            WHERE
                user_id=?         
            `,
            [parseInt(user_id), parseInt(app_notifications), parseInt(email_notifications), parseInt(dark_mode), parseInt(user_id)], (err, dbResults) => {
                if (err) {
                    return res.status(500).json({ error: ENV ? err : 1 });
                }
